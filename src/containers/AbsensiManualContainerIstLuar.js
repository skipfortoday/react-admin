...
import { cekUserAfterFinger, getOptUserManualKeluarIst , takeScreenShoot} from "../actions/optAction";
...
class AbsensiManualContainerIstLuar extends Component {
      ...
      client.onmessage = (message) => {
         ...
         if(result.Key == this.state.key){
            ...
            if(result.Status == "done"){
               ...
               this.props.dispatch(cekUserAfterFinger(this.state.UserID, "istkeluar"))
               takeScreenShoot(this.state.UserID+"_ISTKELUAR")
            }
         }
   ...
   componentDidUpdate() {
      if(this.props.getAfterFinger){
         ...
         setTimeout(() => {
            takeScreenShoot(this.state.UserID+"_ISTKELUAR_OK")
         }, 300);
      }

      if(this.props.errorgetAfterFinger){
         takeScreenShoot(this.state.UserID+"_ISTKELUAR_ERR")
                  ...
                  <Col md={7} style={{ paddingLeft: "5px", paddingRight: "5px" }}>
                     <div style={{ backgroundColor: "#f9a826" }} class="p-2 mb-2">
                        <Container>
                           <Row>
                              <Col md={6}>
                                 <h3 class="text-left mt-2 mb-2">Istirhat Keluar</h3>
                              </Col>
                              <Col md={6}>
